...
class CNN(BaseNetwork):
    def __init__(self,
        ...
        for idx in range(num_blocks):
            out_channels = int(initial_out_channels * filter_growth_rate ** idx)

            self.model.append(ConvBlock(in_channels, out_channels, 
        ...
        self.model.append(ConvBlock(in_channels, 64, 3, 1, act))
        self.model.append(ConvBlock(64, 48, 3, 1, act))
        self.model.append(ConvBlock(48, 32, 4, 1, act))
        self.model.append(ConvBlock(32, 16, 4, 1, act))
        self.model.append(torch.nn.AdaptiveMaxPool2d(2))
        
        self.model.append(torch.nn.Conv2d(16, final_channel, 2, padding=0))
        ...
        if final_layer.lower() == 'linear':
            ...
            raise ValueError(f"Invalid value for final_layer: {final_layer}, it should be 'linear', or 'nlrl'")
