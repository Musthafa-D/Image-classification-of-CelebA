...
class Optuna(BaseOptunaParamOptimizer):
    ...
    def _objective(self, trial):
        ...
        # Classifier
        network = CNN("Classifier", **self.network_config).to(DEVICE)
    ...
    def eval_study(self):
        ...
        fig = optuna.visualization.plot_parallel_coordinate(
            self.study, 
            params=["learning_rate_exp", "learning_rate_exp_l", "weight_decay_rate", "num_blocks"])
        figures_list.append(fig)
        ...
        ax.set_title("test accuracy vs learnable Parameter")
        ax.grid(True)
        ax.set_yticks(range(0, 101, 10))
        self.fig_storage.store(fig, "param_acc_plot")
        ...
        ax.grid(True)
        ax.set_yticks(range(0, 101, 10))
        self.fig_storage.store(fig, "lr_acc_plot")
    ...
    def eval_metrics(self):
        ...
        for trial in self.study.trials:
            ...
            model = CNN("Classifier",
                    initial_out_channels=trial.params['initial_out_channels'],
                    ...
                    activation_function=self.network_config["activation_function"]).to(DEVICE)
        ...
        with open(os.path.join(self.result_folder, "study_info.txt"), "a") as file:
            file.write(f"Time of study excluding metrics calculation: {str(self.duration_metrics)[:-7]}\n")
